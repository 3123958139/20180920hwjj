...
class FillNa(object):
    ...
    def __init__(self, symbols, timedelta, database, table, start, end):
        """
        """
        self._engine    = create_engine('mysql+pymysql://ops:ops!@#9988@47.74.179.216:3308/david')
        ...
        with self._engine.connect() as con:
            for symbol in self._symbols:
                ...
                for t, n in missingTsDF.values:
                    ...
                    """ % (self._database, self._table, symbol, str(t - pd.Timedelta(1 * int(n), unit='h')))
                    df = pd.read_sql(sql, con)
                    for i in range(1, int(n)):
                        ts           = str(t - pd.Timedelta(1 * int(n), unit='h') + pd.Timedelta(1 * i, unit='h'))
...
if __name__ == '__main__':
    ...
    timedelta = pd.Timedelta('0 days 01:00:00')
