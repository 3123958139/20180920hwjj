...
class StatArbHelper:
    ...
    def __updateZScore(self):
        if self.__spread is not None and self.__spreadMean is not None and self.__spreadStd is not None:
            self.__zScore = (self.__spread - self.__spreadMean) / \
                float(self.__spreadStd)
...
class StatArb(strategy.BacktestingStrategy):
    ...
    def onBars(self, bars):
        ...
        self.__spread.appendWithDateTime(
            bars.getDateTime(), self.__statArbHelper.getSpread())
        self.__hedgeRatio.appendWithDateTime(
            bars.getDateTime(), self.__statArbHelper.getHedgeRatio())
        ...
        if bars.getBar(self.__i1) and bars.getBar(self.__i2):
            ...
            if zScore is not None:
                currentPos = abs(self.getBroker().getShares(
                    self.__i1)) + abs(self.getBroker().getShares(self.__i2))
...
def main(plot):
    ...
    if plot:
        ...
        plt.getOrCreateSubplot("hedge").addDataSeries(
            "Hedge Ratio", strat.getHedgeRatioDS())
        plt.getOrCreateSubplot("spread").addDataSeries(
            "Spread", strat.getSpreadDS())
