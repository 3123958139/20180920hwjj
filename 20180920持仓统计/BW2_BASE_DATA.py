            ...
            for label in js[exchange].keys():
                for currency in js[exchange][label].keys():
                    try:
                        ...
                        continue
    df = pd.read_csv('BW2.csv')
    df['usdt'] = df['holding']*df['price']
    btc_usdt = getBtcUsdt()
    df['btc'] = df['usdt']/btc_usdt
    return df

def writeBW2DataToMysql():
    engine = create_engine('mysql://ops:ops!@#9988@47.74.249.179:3308/david')
    with engine.connect() as con:
        df = main()
        df.to_sql('BW2_BASE_DATA', con, schema='david', index=False, if_exists='append', chunksize=500)
        return 'finished'

if __name__ == '__main__':    
    print(writeBW2DataToMysql())
