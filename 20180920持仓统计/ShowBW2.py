...
class ShowBW2(object):
    ...
    def _df(self):
        ...
        FROM david.BW2_BASE_DATA WHERE ts = 
        (SELECT DISTINCT ts FROM david.BW2_BASE_DATA ORDER BY ts DESC LIMIT 1);
        """
    ...
    def main(self):
        ...
        df = df[columns]
        # CREATE TABLE IF NOT EXISTS david.BW2_STAT(ts datetime DEFAULT NULL, UNIQUE KEY ts (ts) USING BTREE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        query = """
        select COLUMN_NAME from information_schema.columns where table_name='BW2_STAT';
        """
        ...
        if len(diff) > 0:
            for dif in diff:
                query = """ALTER TABLE david.BW2_STAT ADD COLUMN %s DOUBLE DEFAULT NULL;                
                """ % dif
        ...
        try:
            ...
            if y == 'y':
                df.to_sql('BW2_STAT', self._con, schema='david', index=False, if_exists='append', chunksize=500)
        except:
            raise Exception
        self._end()
